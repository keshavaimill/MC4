...
export default function Reports() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Reports & Emails</h1>
        <p className="text-sm text-muted-foreground">Download CSV reports or email them directly from the backend</p>
      </div>
        ...
        <ChartContainer title="Available Reports" subtitle="Download or email any report for the current filter period">
          <div className="space-y-3">
            {reportDefinitions.map((report) => (
              <div
                key={report.id}
                className="flex items-center justify-between rounded-lg border border-border bg-accent/20 p-4 transition-colors hover:bg-accent/40"
              >
